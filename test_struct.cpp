...
// [B] required for Bilevel image, [G] required for grayscale image, [P] pallete color, [R] RGB. [1] Byte type [3] short type [4] long type [5] 5 type
struct Ifd{
	std::uint16_t nEntry;
	IfdEntry ImageWidth = IfdEntry(0x0100);			// [B] value = image # of columns
	IfdEntry ImageLength = IfdEntry(0x0101);		// [B] value = image # of rows
	IfdEntry BitsPerSample = IfdEntry(0x0102);		//    [G][R][2] the num of bits per component, value = 4 or 8 (16 or 256 shades), or = 8,8,8 for RGB
	IfdEntry Compresssion = IfdEntry(0x0103);		// [B][2] value 1 = no compression

	IfdEntry Color = IfdEntry(0x0106);				// [B][2] (or PhotometricInterpretation) value 0 = white is zero, 1 = black is zero, 2 = RGB, 3 = palette color
	IfdEntry StripOffsets = IfdEntry(0x0111);		// [B] value = for each strip, the byte offset of that strip
	IfdEntry SamplesPerPixel = IfdEntry(0x0115);	//       [R][2] the number of components per pixel, value = 3 for RGB (or >3 for something special)
	...
	// resolution and unit useful for applications such as page layout. pixel editors do not typically care.
	...
	IfdEntry PlanarConfiguration = IfdEntry(0x011C);	//   [3] value: 1 = Chunky format, RGBRGBRGB. 2 = Planar format (not widespread, not recommended, no need to support)
	// IfdEntry ResolutionUnit = IfdEntry(0x0128);	// [B][2] value: 1 = no measurement, 2=inch (default), 3=centimeter
	// IfdEntry ColorMap = IfdEntry(0x0140);		//    [P][2] for palette color, typeCount = 3*(2**BitsPerSample)

	IfdEntry SampleFormat = IfdEntry(0x0153);		//  [2] value 1=unsigned integer, 2=two's complement signed integer, 3=IEEE floating point, 4=undefined. Note, the size of sample is still done by BitsPerSample.
...
struct Ifd_readable {
	...
	std::vector<std::uint32_t>* BitsPerSample
}
